...
type TrechGenericRollCalculatorProps = {
  plusDice: string;
  minusDice: string;
  positiveModifier: string;
  negativeModifier: string;
  errorMessage: string;
  results: TrechGenericResults | null;
  debug: {
    rolls: number[];
    selectedRolls: number[];
  };
...
const formatSelectionLabel = (mode: TrechGenericResults['selectionMode']) => {
  if (mode === 'highest') {
    return 'Highest two';
  }
  if (mode === 'lowest') {
...
export default function TrechGenericRollCalculator({
  plusDice,
  minusDice,
  positiveModifier,
  negativeModifier,
  ...
  onRoll,
  onBack,
}: TrechGenericRollCalculatorProps) {
  return (
