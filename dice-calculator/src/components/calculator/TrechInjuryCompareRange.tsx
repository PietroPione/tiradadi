...
type CompareConfig = TrechInjuryInputs & {
  id: string;
  label: string;
...
type TrechInjuryCompareRangeProps = TrechInjuryInputs & {
  onBack: () => void;
  backLabel?: string;
  rightSlot?: ReactNode;
...
const fieldLabels: Record<keyof RangeFieldValues, string> = {
  ...
  armorPositiveModifier: 'Armor +',
  armorNegativeModifier: 'Armor -',
};

const parseNumber = (value: string) => Number.parseInt(value, 10);

const buildCompareConfig = (base: TrechInjuryInputs, index: number): CompareConfig => ({
  ...base,
...
export default function TrechInjuryCompareRange({
  plusDice,
  minusDice,
  positiveModifier,
  negativeModifier,
...
}: TrechInjuryCompareRangeProps) {
  const baseInputs: TrechInjuryInputs = {
    plusDice,
    minusDice,
    positiveModifier,
    negativeModifier,
  ...
  const buildSeries = () => {
    const rangeSeries = compareItems.flatMap((item) => {
      ...
      return values.map((value) => {
        const inputs: TrechInjuryInputs = {
          ...
          withThreeDice: item.withThreeDice,
          targetArmor: item.targetArmor,
          noArmorSave: item.noArmorSave,
          armorPositiveModifier: item.armorPositiveModifier,
          armorNegativeModifier: item.armorNegativeModifier,
        };
  ...
  const handleGenerate = () => {
    ...
    const baseDice = withThreeDice ? 3 : 2;
    const armorBase = noArmorSave
      ? 0
      : Math.max(0, parseNumber(targetArmor) + parseNumber(armorPositiveModifier) - parseNumber(armorNegativeModifier));
    const baseMax = Math.max(
      0,
      baseDice * 6 + parseNumber(positiveModifier) - parseNumber(negativeModifier) - armorBase,
    );
    const seriesData = rangeSeries.map((entry) => {
      const entryBaseDice = entry.inputs.withThreeDice ? 3 : 2;
      const armorValue = entry.inputs.noArmorSave
        ? 0
        : Math.max(
          0,
          parseNumber(entry.inputs.targetArmor)
            + parseNumber(entry.inputs.armorPositiveModifier)
            - parseNumber(entry.inputs.armorNegativeModifier),
        );
      const maxValue = Math.max(
        0,
        entryBaseDice * 6
          + parseNumber(entry.inputs.positiveModifier)
          - parseNumber(entry.inputs.negativeModifier)
          - armorValue,
      );
      return { name: entry.name, inputs: entry.inputs, max: maxValue };
    });
    const overallMax = Math.max(baseMax, ...seriesData.map((item) => item.max));
    const simulate = (inputs: TrechInjuryInputs) => {
      ...
      const baseDiceCount = inputs.withThreeDice ? 3 : 2;
      const netDice = parsedPlus - parsedMinus;
      const totalDice = baseDiceCount + Math.abs(netDice);
      const armorValue = inputs.noArmorSave
        ? 0
        : Math.max(
          0,
          parseNumber(inputs.targetArmor)
            + parseNumber(inputs.armorPositiveModifier)
            - parseNumber(inputs.armorNegativeModifier),
        );
      for (let i = 0; i < iterations; i += 1) {
        ...
        let selected = sorted;
        if (netDice > 0) {
          selected = sorted.slice(-baseDiceCount);
        } else if (netDice < 0) {
          selected = sorted.slice(0, baseDiceCount);
        }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <Card className="px-4 py-5 sm:px-6 sm:py-6">
          <CardHeader title="Step 1: Injury base values" subtitle="Set your baseline for comparison" />
          <div className="mt-4 space-y-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
              ...
              />
              <InputField
                id="trechInjuryCompareMinusDice"
                label="-Dice"
                value={minusDice}
                min="0"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
              <InputField
                id="trechInjuryComparePositiveMod"
              ...
              />
              <InputField
                id="trechInjuryCompareNegativeMod"
            ...
            <InputField
              id="trechInjuryCompareTargetArmor"
              label="Target armor"
              value={targetArmor}
              min="0"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
              <InputField
                id="trechInjuryCompareArmorPositive"
              ...
              />
              <InputField
                id="trechInjuryCompareArmorNegative"
